...
class Fire(Sprite):
    def __init__(self, surf, pos, groups, player):
        ...
        self.timer = Timer(125, autostart = True, func = self.kill)
...
class Player(AnimatedSprite):
        ...
        for sprite in self.collision_sprites:
            if sprite.rect.colliderect(self.rect):
                ...
                if direction == 'vertical':
                    ...
                    self.direction.y = 0 # resets gravity on vertical collisions 
    ...
    def animate(self, dt):
        if self.direction.x:
            self.frame_index += self.animation_speed * dt
            self.flip = self.direction.x < 0 # if moving right:True, if moving left:False
        else:
            self.frame_index = 0
